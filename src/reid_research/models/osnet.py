"""OSNet model architecture ported from TorchReID.

Reference:
    - Zhou et al. Omni-Scale Feature Learning for Person Re-Identification. ICCV, 2019.
    - Zhou et al. Learning Generalisable Omni-Scale Representations for Person Re-ID. TPAMI, 2021.
"""
...
__all__ = ["osnet_x1_0", "osnet_ibn_x1_0", "OSNet"]

# Pretrained weight URLs (Google Drive)
PRETRAINED_URLS = {
...
class OSNet(nn.Module):
    ...
    def _construct_fc_layer(
        self, fc_dims: int | list | None, input_dim: int, dropout_p: float | None = None
    ) -> nn.Sequential | None:
...
def init_pretrained_weights(model: nn.Module, key: str = "") -> None:
    """Initialize model with pretrained weights from Google Drive."""
    ...
    if not os.path.exists(cached_file):
        if key not in PRETRAINED_URLS:
            warnings.warn(f"No pretrained weights for {key}")
            return
        gdown.download(PRETRAINED_URLS[key], cached_file, quiet=False)

    state_dict = torch.load(cached_file, weights_only=False)
    ...
    if len(matched_layers) == 0:
        ...
        if len(discarded_layers) > 0:
            print(
                f"** The following layers are discarded due to unmatched keys or "
                f"layer size: {discarded_layers}"
            )


def osnet_x1_0(
    num_classes: int = 1000, pretrained: bool = True, loss: str = "softmax", **kwargs
) -> OSNet:
    """OSNet x1.0 (standard size, 512-dim features)."""
    model = OSNet(
        num_classes,
...
def osnet_ibn_x1_0(
    ...
    """OSNet x1.0 with Instance Normalization (IBN-Net style)."""
    model = OSNet(
        num_classes,
        blocks=[OSBlock, OSBlock, OSBlock],
        layers=[2, 2, 2],
